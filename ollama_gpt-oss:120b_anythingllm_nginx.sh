...
wait_for_ollama() {
  ...
  while (( attempt <= max_attempts )); do
    ...
    if [[ "${HTTP_CODE}" == "200" ]]; then
      echo "✅ Ollama is online (HTTP 200 from /api/tags)."
      return 0
    fi
  ...
  echo "❌ Ollama did not become ready in time (>${MAX_WAIT_MINUTES} minutes)."
  docker logs ollama --tail 200 2>&1 || true
  return 1
}
...
if [[ "${START_NOW}" =~ ^[Yy]$ ]]; then
  ...
  echo
  echo "===================================================================="
  echo " 🎉 Deployment complete & Ollama is ONLINE."
  echo "--------------------------------------------------------------------"
...
echo
echo "Done. One script to rule them all (Ollama edition). 🧙‍♂️"
